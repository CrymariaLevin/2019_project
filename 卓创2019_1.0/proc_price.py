...
def price_test(datadir):  # 单独跑测试库
    ppf_t.init_data()
    ppf_t.job_datadir(datadir)
    ppm_t.init_data()
    ppm_t.job_datadir(datadir)

def price_formal(datadir):  # 单独跑正式库
    ppf.init_data()
    ppf.job_datadir(datadir)
    ppm.init_data()
    ppm.job_datadir(datadir)
...
if __name__ == "__main__":
    ...
    #print(DIR)
    t1 = threading.Thread(target = price_test, args = (DIR,)) #注意传入的参数是元组格式，单个参数要加逗号
    t2 = threading.Thread(target = price_formal, args = (DIR,))
    t1.start()
    t2.start()
    t1.join()
    t2.join()
